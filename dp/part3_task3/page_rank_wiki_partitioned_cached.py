...
if __name__ == "__main__":
	...
	if len(sys.argv) != 4:
		print("run_task3_wiki.sh <masterIP:port> <HDFS inputFile> <HDFS outputFile> <#iterations>")
		sys.exit(-1)
	...
	.appName("enwiki-partitioned-cached wiki")\
	...
	links = lines.map(lambda ids: parseNeighborsWiki(ids)).filter(lambda x: x).distinct().groupByKey().repartition(30).cache()
	...
	for iteration in range(iterations):
		...
		# Calculates nodeID contributions to the rank of other nodeIDs.
		contribs = links.join(ranks).flatMap( lambda id_ids_rank: computeContribs(id_ids_rank[1][0], id_ids_rank[1][1])).repartition(200)
