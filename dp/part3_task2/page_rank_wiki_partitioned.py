...
if __name__ == "__main__":
	...
	if len(sys.argv) != 4:
		print("run_task2_wiki.sh <masterIP:port> <HDFS inputFile> <HDFS outputFile> <#iterations>")
		sys.exit(-1)
	...
	.appName("enwiki-partitioned-30-200 wiki")\
	...
	links = lines.map(lambda ids: parseNeighborsWiki(ids)).filter(lambda x: x).distinct().groupByKey().repartition(30)
